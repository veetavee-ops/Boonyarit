...
const { Message, User, Group, Setting, Admin, AdminGroup, PaymentVerification, LedgerBalanceEntry } = require('../models/index');
...
const { extractPaymentDocuments, matchPaymentItems, extractReceiptSummary, extractTransferSlip, extractWrittenBalance } = require('../services/aiService');
const { getSearchKeyword, getSummarizeKeyword, escapeRegex, matchSummarizeCommand, buildSearchReply, buildSummarizeReply, resolveVisionProviderChain, buildPaymentVerifyReply, buildReceiptSummaryReply, buildLedgerBalanceReply, buildWrittenBalanceCheckReply } = require('../services/botCommandService');
...
// คำสั่ง "เช็คสมุด" ของฟีเจอร์เช็คยอดสมุดบัญชี — ส่งรูปหน้าสมุดมาเทียบยอด (หรือใช้ตั้งยอดเริ่มต้นถ้า
// ยังไม่มีรายการใดเลยในระบบ) คนละคำสั่งกับ receipt_summary_keyword ด้านบน — ตั้งค่าแยกได้เองใน admin panel
async function getLedgerBookCheckKeyword() {
    const s = await Setting.findByPk('ledger_book_check_keyword');
    return (s?.value || 'เช็คสมุด').trim();
}
...
// Ledger balance check (เช็คยอดสมุดบัญชี ยืม-คืนเงิน) — คนละฟีเจอร์กับ Payment Verification ด้านบน
// รอสลิปโอนเงิน 1-2 รูปจากคนเดียวกัน แต่ต่างจาก pendingPaymentImages ตรงที่ครบเวลาแล้ว "ประมวลผลด้วย
// รูปเท่าที่มี" แทนการทิ้งไปเฉยๆ เพราะสลิปใบเดียวก็มักเพียงพอต่อการอ่านข้อมูลอยู่แล้ว
const pendingLedgerBalanceImages = new Map(); // key: `${userId}-${groupId}`
const LEDGER_IMAGE_TIMEOUT = 8000;

// Session "เช็คสมุด" — เปิดด้วย keyword แล้วรอรูปหน้าสมุดบัญชีรูปเดียว (คนละ Map จาก pendingReceiptSummary)
const pendingLedgerBookCheck = new Map(); // key: `${groupId}_${userId}`
const LEDGER_BOOK_CHECK_IDLE_TIMEOUT = 2 * 60 * 1000; // 2 นาที ให้เวลาถ่ายรูปส่ง
...
3) ตรวจสอบการโอน-ตั้งเบิก (AI Vision) — ส่งรูปตั้งเบิก + สกรีนช็อตธนาคารเข้ากลุ่มที่ติดธงไว้ AI จับคู่ยอดอัตโนมัติ ตอบกลับทันที + เก็บ ledger ให้ดูใน dashboard
...
async function handleEvent(event, io) {
    ...
    if (message.type === 'text') {
        ...
        // คำสั่ง "เช็คสมุด" ของฟีเจอร์เช็คยอดสมุดบัญชี (ยืม-คืนเงิน) — เฉพาะกลุ่มที่ติดธง
        // isLedgerBalanceGroup เท่านั้น เป็นคนละฟีเจอร์กับสรุปบิลซื้อของด้านบนโดยสิ้นเชิง
        if (sourceType === 'group' && groupId) {
            const ledgerBookCheckKeyword = await getLedgerBookCheckKeyword();
            if ((message.text || '').trim() === ledgerBookCheckKeyword) {
                const group = await Group.findByPk(groupId);
                if (group?.isLedgerBalanceGroup) {
                    await handleLedgerBookCheckStart(event, userId, groupId);
                    return;
                }
            }
        }
...
async function handleImageMessage(event, userId, groupId, sourceType, message, io, folderName) {
    ...
    if (sourceType === 'group' && groupId) {
        ...
        // กลุ่มที่ติดธง isLedgerBalanceGroup — คนละฟีเจอร์จาก 2 อันบนโดยสิ้นเชิง ถ้ามี session
        // "เช็คสมุด" เปิดอยู่ ให้ไปตรวจ/ตั้งยอดจากรูปสมุดแทน ไม่งั้นถือเป็นสลิปโอนเงินธุรกรรมใหม่
        // ทั้ง 2 ทางไม่บันทึกรูปเป็นแชทปกติเหมือนกัน
        if (group?.isLedgerBalanceGroup) {
            const bookCheckKey = `${groupId}_${userId}`;
            if (pendingLedgerBookCheck.has(bookCheckKey)) {
                return await handleLedgerBookCheckImage(event, groupId, userId, message, bookCheckKey);
            }
            return await handleLedgerBalanceImage(event, userId, groupId, message, io, groupKey);
        }
    }
...
async function processPaymentVerification(groupId, userId, images, replyToken, io) {
    try {
        ...
        await client.replyMessage(replyToken, {
            type: 'text',
            text: buildPaymentVerifyReply(extracted, matchResults, overallStatus),
        }).catch(() => {});
    } catch (err) {
...
async function processReceiptSummary(images, replyToken) {
    try {
        ...
        await client.replyMessage(replyToken, { type: 'text', text: truncateForLine(buildReceiptSummaryReply(extracted)) + BOT_COMMAND_NOTICE })
...
// ─── Ledger Balance Check — เช็คยอดสมุดบัญชี (ยืม-คืนเงิน) แยกจาก Payment Verification สิ้นเชิง ──
// รอสลิปโอนเงิน 1-2 รูปจากคนเดียวกัน แล้วส่งตรวจสอบ+คำนวณยอดต่อเนื่อง (ระบบจำยอดเอง ไม่อ่านจากรูป
// สมุดใหม่ทุกครั้ง) — ต่างจาก payment-verify ตรงที่ครบเวลาแล้ว "ประมวลผลด้วยรูปเท่าที่มี" (1 หรือ 2 รูป)
// แทนการทิ้งไปเฉยๆ เพราะสลิปใบเดียวก็มักเพียงพอต่อการอ่านข้อมูลอยู่แล้ว
async function handleLedgerBalanceImage(event, userId, groupId, message, io, groupKey) {
    const buffer = await downloadAsBuffer(message.id);
    const imageData = { lineMessageId: message.id, buffer, timestamp: new Date(event.timestamp) };
    const replyToken = event.replyToken;

    if (pendingLedgerBalanceImages.has(groupKey)) {
        const pending = pendingLedgerBalanceImages.get(groupKey);
        clearTimeout(pending.timer);
        pending.images.push(imageData);

        if (pending.images.length >= 2) {
            pendingLedgerBalanceImages.delete(groupKey);
            await processLedgerBalanceEntry(groupId, userId, pending.images.slice(0, 2), replyToken, io);
            return;
        }
        pending.timer = setTimeout(() => handleLedgerBalanceTimeout(groupKey), LEDGER_IMAGE_TIMEOUT);
    } else {
        pendingLedgerBalanceImages.set(groupKey, {
            userId, groupId, io,
            images: [imageData],
            replyToken,
            timer: setTimeout(() => handleLedgerBalanceTimeout(groupKey), LEDGER_IMAGE_TIMEOUT),
        });
    }
}

// ครบเวลาแล้วยังไม่ครบ 2 รูป — ประมวลผลด้วยรูปเท่าที่มี (ต่างจาก payment-verify ที่ทิ้งไปเฉยๆ เพราะ
// สลิปโอนเงินใบเดียวมักมีข้อมูลครบพอให้อ่านได้อยู่แล้ว ไม่จำเป็นต้องรอครบ 2 รูปเสมอไป)
async function handleLedgerBalanceTimeout(groupKey) {
    const pending = pendingLedgerBalanceImages.get(groupKey);
    pendingLedgerBalanceImages.delete(groupKey);
    if (!pending) return;
    await processLedgerBalanceEntry(pending.groupId, pending.userId, pending.images, pending.replyToken, pending.io);
}

// แปลง string วันที่/เวลาที่ AI อ่านได้จากสลิปเป็น Date จริง — parse ไม่ได้ (เช่น พ.ศ. รูปแบบแปลกๆ)
// ให้คืน null แทนการปล่อยให้ Sequelize insert ค่า Invalid Date แล้ว error ทั้งก้อน (ฟิลด์นี้ใช้แค่
// แสดงผล/ตรวจสอบ ไม่ใช่ตัวเรียงลำดับ chain อยู่แล้ว ไม่กระทบความถูกต้องของยอดคงเหลือ)
...
async function processLedgerBalanceEntry(groupId, userId, images, replyToken, io) {
    try {
        const group = await Group.findByPk(groupId);
        ...
        if (!referenceName) {
            await client.replyMessage(replyToken, {
                type: 'text',
                text: '⚠️ กลุ่มนี้ยังไม่ได้ตั้งค่า "ชื่ออ้างอิง" สำหรับเช็คทิศทางเงิน กรุณาตั้งค่าที่ Dashboard ก่อนใช้งานฟีเจอร์นี้ครับ',
            }).catch(() => {});
            return;
        }

        const visionChain = await resolveVisionProviderChain();
        const extracted = await extractTransferSlip(images.map(img => img.buffer), visionChain, referenceName);

        if (!extracted.direction) {
            await client.replyMessage(replyToken, {
                type: 'text',
                text: `⚠️ ไม่สามารถระบุทิศทางเงิน (ยืม/คืน) ได้จากสลิปนี้ครับ${extracted.note ? '\n' + extracted.note : ''}\nกรุณาตรวจสอบว่าชื่อ "${referenceName}" ปรากฏชัดเจนในสลิป หรือแจ้งแอดมินให้บันทึกรายการนี้ด้วยมือแทน`,
            }).catch(() => {});
            return;
        }

        const latestEntry = await LedgerBalanceEntry.findOne({
            where: { groupId },
            order: [['submittedAt', 'DESC']],
        });
        if (!latestEntry) {
            const keyword = await getLedgerBookCheckKeyword();
            await client.replyMessage(replyToken, {
                type: 'text',
                text: `⚠️ ยังไม่ได้ตั้งยอดเริ่มต้นของบัญชีนี้เลยครับ กรุณาพิมพ์ "${keyword}" แล้วส่งรูปสมุดบัญชีหน้าล่าสุดก่อน เพื่อตั้งยอดตั้งต้น`,
            }).catch(() => {});
            return;
        }
        ...
        const slipImagePaths = [];
        for (let i = 0; i < images.length; i++) {
            try {
                const gcsPath = buildGCSPath(images[i].lineMessageId + `-slip${i}`, '.jpg', 'ledger-balance');
                await uploadToGCS(images[i].buffer, gcsPath, '.jpg');
                slipImagePaths.push(gcsPath);
            } catch (e) {
                console.error('❌ Ledger balance slip GCS fail:', e.message);
            }
        }

        const entry = await LedgerBalanceEntry.create({
            groupId,
            submittedBy: userId,
            submittedAt: new Date(),
            ...
            occurredAt: safeParseDate(extracted.occurredAt),
            counterName: extracted.direction === 'in' ? extracted.fromName : extracted.toName,
            referenceNameMatched: extracted.referenceNameMatched,
            slipImagePaths,
        ...
        await client.replyMessage(replyToken, {
            type: 'text',
            text: buildLedgerBalanceReply(entry),
        }).catch(() => {});
    } catch (err) {
        console.error('❌ Ledger Balance Entry Error:', err.message);
        alertError('Ledger Balance Entry', err.message);
        await client.replyMessage(replyToken, {
            type: 'text',
            text: '❌ บันทึกรายการไม่สำเร็จ (อ่านรูปไม่ได้) กรุณาลองส่งรูปใหม่อีกครั้ง',
        }).catch(() => {});
    }
}

// ─── "เช็คสมุด" — ถ่ายรูปหน้าสมุดบัญชีมาเทียบยอด (หรือใช้ตั้งยอดเริ่มต้นถ้ายังไม่มีรายการใดในระบบเลย) ──
async function handleLedgerBookCheckStart(event, userId, groupId) {
    const sessionKey = `${groupId}_${userId}`;
    if (pendingLedgerBookCheck.has(sessionKey)) {
        clearTimeout(pendingLedgerBookCheck.get(sessionKey).timer);
    }
    pendingLedgerBookCheck.set(sessionKey, {
        timer: setTimeout(() => pendingLedgerBookCheck.delete(sessionKey), LEDGER_BOOK_CHECK_IDLE_TIMEOUT),
    });
    await client.replyMessage(event.replyToken, {
        type: 'text',
        text: '📖 ส่งรูปหน้าสมุดบัญชี (แถวล่างสุดที่เขียนไว้) มาได้เลยครับ',
    }).catch(() => {});
}

// เรียกจาก handleImageMessage เมื่อกลุ่มติดธง isLedgerBalanceGroup และมี session "เช็คสมุด" เปิดอยู่ของ user นี้
async function handleLedgerBookCheckImage(event, groupId, userId, message, sessionKey) {
    const session = pendingLedgerBookCheck.get(sessionKey);
    if (!session) return; // session หมดเวลาไปพอดีระหว่างนี้
    clearTimeout(session.timer);
    pendingLedgerBookCheck.delete(sessionKey);

    const replyToken = event.replyToken;
    try {
        const buffer = await downloadAsBuffer(message.id);
        const visionChain = await resolveVisionProviderChain();
        const extracted = await extractWrittenBalance(buffer, visionChain);

        const gcsPath = buildGCSPath(message.id + '-book', '.jpg', 'ledger-balance');
        await uploadToGCS(buffer, gcsPath, '.jpg').catch((e) => console.error('❌ Ledger book image GCS fail:', e.message));
        ...
        if (!latestEntry) {
            // ยังไม่มีรายการไหนเลย — รูปนี้กลายเป็นยอดตั้งต้น (seed)
            const seedEntry = await LedgerBalanceEntry.create({
                groupId,
                submittedBy: userId,
                submittedAt: new Date(),
                entryType: 'seed',
                previousBalance: null,
                calculatedBalance: extracted.balance,
                writtenBalanceImagePath: gcsPath,
                writtenBalanceExtracted: extracted.balance,
                writtenBalanceCheckedAt: new Date(),
                writtenBalanceCheckedBy: userId,
                matchesWrittenBalance: true,
                status: 'confirmed',
                extractedRaw: extracted,
                model: extracted.model,
            });
            const balanceText = Number(seedEntry.calculatedBalance).toLocaleString('th-TH', { minimumFractionDigits: 2 });
            await client.replyMessage(replyToken, {
                type: 'text',
                text: `✅ ตั้งยอดเริ่มต้นแล้ว: ${balanceText} บาท\nส่งสลิปโอนเงินครั้งถัดไปได้เลยครับ ระบบจะคำนวณยอดต่อเนื่องให้เอง`,
            }).catch(() => {});
            return;
        }

        // มีรายการอยู่แล้ว — เทียบยอดที่คำนวณได้ (ของแถวล่าสุด) กับยอดที่เขียนในรูปนี้
        ...
        await latestEntry.update({
            ...
            writtenBalanceCheckedAt: new Date(),
            writtenBalanceCheckedBy: userId,
            matchesWrittenBalance: matches,
            status: matches ? 'confirmed' : 'mismatch',
        });

        await client.replyMessage(replyToken, {
            type: 'text',
            text: buildWrittenBalanceCheckReply(latestEntry),
        }).catch(() => {});
    } catch (err) {
        console.error('❌ Ledger Book Check Error:', err.message);
        alertError('Ledger Book Check', err.message);
        await client.replyMessage(replyToken, {
            type: 'text',
            text: '❌ อ่านรูปสมุดบัญชีไม่สำเร็จ กรุณาลองส่งรูปใหม่อีกครั้ง',
        }).catch(() => {});
    }
}
